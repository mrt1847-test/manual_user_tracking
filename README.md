...
```bash
python scripts/sheets_to_json.py --area SRP --module "먼저 둘러보세요" --overwrite
python scripts/sheets_to_json.py --area SRP --sheet --overwrite
```

## 사용 방법

1. `플랫폼`, `환경`, `영역`, `모듈`, `nth`, `이벤트`를 선택합니다.
2. `상품번호`, 검색어/카테고리, 광고 여부, 가격 정보를 필요에 따라 입력합니다.
3. raw payload 원문을 붙여넣습니다.
4. `검증 실행`을 누르면 필드별 `PASS/FAIL` 결과를 확인할 수 있습니다.
